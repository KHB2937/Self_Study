...
for i, review in enumerate(review_elements):
    ...
    try:
        # 리뷰에서 별점 요소를 찾습니다.
        rating_element = review.find_element(By.XPATH, ".//div[contains(@class, 'gyAGI')]/em")

        # 요소의 내부 HTML에 '<em>' 태그가 포함되어 있다면, 그 태그 안에 별점 값이 있습니다.
        if '<em>' in rating_element.get_attribute('innerHTML'):
            # '<em>' 태그를 찾아 별점 값을 가져옵니다.
            rating_element = rating_element.find_element(By.XPATH, ".//em")
            # <em> 태그의 내용을 별점 값으로 사용합니다.
            rating = rating_element.text
            # rating = rating_element.get_attribute('innerHTML')
        else:
            # 그렇지 않다면, 별점은 img 태그의 alt 속성 값에 있습니다.
            rating_image = rating_element.find_element(By.TAG_NAME, 'img')
            rating = rating_image.get_attribute('alt')
    ...
    data.append([reviewer, content, rating])
